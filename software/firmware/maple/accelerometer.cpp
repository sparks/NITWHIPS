...
Accelerometer::Accelerometer() {
	...
	for(uint8 Accelerometer::i = 0;i < AXES;i++) {
		position[i] = 0;
...
void Accelerometer::who_am_i() {
	uint8 Accelerometer::res = accel_read(WHO_AM_I);
	SerialUSB.println(res, HEX);
}

uint8 Accelerometer::accel_write(uint8 Accelerometer::addr, uint8 Accelerometer::data) {
...
uint8 Accelerometer::accel_read(uint8 Accelerometer::addr) {
	Wire.requestFrom(ADDR_DEVICE, 1);
	return Wire.receive();
}

uint8 Accelerometer::accel_multi_read(uint8 Accelerometer::addr, uint8 Accelerometer::num, uint8 Accelerometer::data[]) {
	Wire.beginTransmission(ADDR_DEVICE);
	Wire.send(addr);
	Wire.endTransmission();

	uint8 Accelerometer::rx_len = Wire.requestFrom(ADDR_DEVICE, num);
	for(uint8 Accelerometer::i = 0;i < num;i++) data[i] = Wire.receive();
	return rx_len;
}
...
void Accelerometer::mark() {
	for(uint8 Accelerometer::i = 0;i < MARK_LEN;i++) SerialUSB.print(0xFF, BYTE);
}
...
void Accelerometer::computerPosition() {
	return;
}

void Accelerometer::pollAndUpdate() {
	//Read accel data
	uint8 Accelerometer::data[AXES*2+1];
	
	for(uint8 Accelerometer::i = 0;i < AXES*2+1;i++) data[i] = 0;
	accel_multi_read(STATUS, AXES*2+1, data);
	
	if((data[0] & ((1 << XDR) | (1 << YDR))) == ((1 << XDR) | (1 << YDR))) { //Wait for new data
		//Parse into signed ints
		for(uint8 Accelerometer::i = 0;i < AXES;i++) accel[i] = 0;
